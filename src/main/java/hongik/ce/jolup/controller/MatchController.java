...
//        model.addAttribute("matchStatuses", MatchStatus.values());
        ...
        if (matchDto.getMatchStatus().equals(MatchStatus.END)) {
            if (matchDto.getScore().getUser1Score() > matchDto.getScore().getUser2Score()) {
                ...
                if (roomDto.getRoomType().equals(RoomType.TOURNAMENT) && !(score.getUser1Score() > score.getUser2Score())) {
                    MatchDto prevMatchDto = matchService.getMatch(matchId);
                    MatchDto nextMatchDto = matchService.findOne(roomDto, prevMatchDto.getRoundNo() + 1L, (long)Math.ceil(prevMatchDto.getMatchNo() / 2.0));
                    while (nextMatchDto != null) {
                        if (prevMatchDto.getMatchNo() % 2 == 1) {
                        ...
                        prevMatchDto = matchService.findOne(roomDto, prevMatchDto.getRoundNo() + 1L, (long)Math.ceil(prevMatchDto.getMatchNo() / 2.0));
                        nextMatchDto = matchService.findOne(roomDto, prevMatchDto.getRoundNo() + 1L, (long)Math.ceil(prevMatchDto.getMatchNo() / 2.0));
                    }
                }
        ...
        if (matchStatus.equals(MatchStatus.END)) {
            if (score.getUser1Score() > score.getUser2Score()) {
                ...
                if (roomDto.getRoomType().equals(RoomType.TOURNAMENT)) {
                    MatchDto nextMatchDto = matchService.findOne(roomDto, matchDto.getRoundNo() + 1L, (long)Math.ceil(matchDto.getMatchNo() / 2.0));
    ...
    /*private void leagueMatchUpdate(Long matchId, Score score, MatchStatus matchStatus) {
        MatchDto matchDto = matchService.getMatch(matchId);
        RoomDto roomDto = matchDto.getRoomDto();
        JoinDto joinDto1 = joinService.findOne(matchDto.getUser1Dto(), matchDto.getRoomDto());
        Result result1 = joinDto1.getResult();
    ...
    private void tournamentMatchUpdate() {
